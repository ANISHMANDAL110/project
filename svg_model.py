...
# SVR Model for 30-day Stock Forecast
import sys
import os
import pandas as pd
import numpy as np
...
def get_paths(symbol):
    ...
    output_file = f'data-uploader/predicted-data/{symbol}-svr-predicted.csv'
...
def main():
    if len(sys.argv) != 2:
        print('Usage: python svg_model.py <SYMBOL>')
        return
    symbol = sys.argv[1]
        ...
        for lag in DAYS_LIST:
            if len(forecasted_values) > lag:
                ...
                if historical_index >= 0:
                    ...
                    next_features.append(df['adjclose'].iloc[-1])
        # Scale next features
        next_features_scaled = scaler.transform(np.array(next_features).reshape(1, -1))
        current_features = next_features_scaled
